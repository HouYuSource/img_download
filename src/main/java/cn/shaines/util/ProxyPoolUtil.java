...
public class ProxyPoolUtil {
    ...
    private static LoggerFactory.Logger logger = LoggerFactory.getLogger(ProxyPoolUtil.class);
    ...
    private List<String> findAllProxy() {
        ...
        try {
            ...
            logger.warn(PublicUtil.getDetailMessage(e));
        }
        return allProxy;
    }
    ...
    private void checkUsefulSet(Collection<String> thisAllProxy) {
        ...
        for(String proxy : this.usefulSet) {
            ...
            ThreadPoolUtil.get().submit(() -> {
                ...
                try {
                    HttpURLConnectionUtil.build("https://www.sogou.com/").setTimeout(3000).setProxy(host_port[0], Integer.valueOf(host_port[1])).execute();
    ...
    public static class CaptureProxyImpl1 implements CaptureProxy{
        ...
        public Collection<String> parse() {
            ...
            for (int i = 1; i < 5; i++){
                String html = HttpURLConnectionUtil.build("http://ip.jiangxianli.com/?page=" + i).execute().getBodyString();
                List<String> list = PublicUtil.regexMatcher(html, "data-url=\"http://(.*)\"\\s*data-unique-id=", List.class);
                if (list.size() == 0){
                    break;
                }
